...
export default async function POST(req: Request) {
    ...
    console.info(
        `[jira/api/chat] request_id=${requestId} model=${model} message_count=${messageCount} prompt_characters=${promptCharacters} thinking_supported=${canThink}`,
    );
    ...
    const stream = createUIMessageStream({
        ...
        onError: (error) => {
            console.error(
                `[jira/api/chat] request_id=${requestId} model=${model} stream_error=${error instanceof Error ? error.message : String(error)}`,
            );
    ...
    console.info(
        `[jira/api/chat] request_id=${requestId} model=${model} duration_ms=${Date.now() - startedAt}`,
    );
